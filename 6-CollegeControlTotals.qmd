...
# Campus-Level Disaggregation

## Load USHE 2025 Campus Enrollment Data

For Home Based College (HBC) travel modeling, we need to disaggregate institution-level forecasts to specific campus locations. We use 2025 actual enrollment from USHE to calculate the percentage of students at each campus, then apply those percentages to all forecast years.

```{python}
# Load USHE 2025 enrollment data
df_ushe_2025 = pd.read_excel(
    "data/USHE 2025 Enrollment/WFRC Data Request_Final.xlsx",
    sheet_name="Sheet1"
)

print(f"Total USHE records loaded: {len(df_ushe_2025)}")
print(f"Columns: {df_ushe_2025.columns.tolist()}")
print(f"\nUnique institutions: {df_ushe_2025['Institution'].unique()}")
print(f"\nUnique terms: {sorted(df_ushe_2025['Term'].unique())}")
print(f"Unique years: {sorted(df_ushe_2025['Academic Year'].unique())}")

# Filter to 2025 academic year
# Note: Term 2 is typically Fall semester in Utah's system
# We'll use Term 2 for Fall 2025 enrollment
df_ushe_fall_2025 = df_ushe_2025[
    (df_ushe_2025['Academic Year'] == 2025) &
    (df_ushe_2025['Term'] == 2)  # Term 2 = Fall
].copy()

print(f"\nFiltered to Academic Year 2025, Term 2 (Fall): {len(df_ushe_fall_2025)} records")
...
print(f"\nUnique campuses after aggregation: {len(df_campus_enrollment)}")
print("\n=== Sample Campus Enrollments ===")
print(df_campus_enrollment.head(15).to_string())
...
## Define Campus Mapping

```{python}
# Map USHE Site names to campus codes for target institutions
CAMPUS_MAPPING = {
    ...
    'Utah Valley University': {
        'Main Campus': 'UVU_Main',
        # Note: UVU_Lehi, UVU_Payson, UVU_Vineyard handled separately
    },
...
SINGLE_CAMPUS = {
    ...
    'Ensign College': 'ENSIGN',
    'Westminster College': 'WESTMIN',
}
```
...
```{python}
# Calculate percentage splits for each institution
campus_percentages = {}

print("\n=== Calculating Campus Percentages ===")

for institution, campus_map in CAMPUS_MAPPING.items():
    print(f"\n{institution}:")

    # Get total enrollment for this institution
    inst_data = df_campus_enrollment[df_campus_enrollment['Institution'] == institution]
    print(f"  Total records found: {len(inst_data)}")

    if len(inst_data) == 0:
        print(f"  WARNING: No data found for '{institution}'")
        print(f"  Available institutions in data: {df_campus_enrollment['Institution'].unique().tolist()}")
        continue

    # Show all sites for this institution
    print(f"  All sites in data:")
    for _, row in inst_data.iterrows():
        print(f"    - '{row['Site']}': {row['Headcount']:,} students")

    # Get enrollment for mapped campuses only
    mapped_campuses = list(campus_map.keys())
    print(f"  Looking for mapped sites: {mapped_campuses}")
    ...
    if total_mapped > 0:
        ...
        print(f"  WARNING: No enrollment found for any mapped campuses")
        print(f"  This usually means site names don't match between data and mapping")

# Display final summary
print("\n" + "="*60)
print("=== Final Campus Split Percentages ===")
print("="*60)
if len(campus_percentages) == 0:
    print("ERROR: No campus percentages calculated!")
    print("Check that institution and site names in CAMPUS_MAPPING match the USHE data exactly")
else:
    for campus, pct in sorted(campus_percentages.items()):
        print(f"{campus}: {pct:.1%}")
```

## Apply Disaggregation to Forecast
...
# Create disaggregated forecast - restructured approach
# First, create a complete year x campus matrix

# Get all unique years from forecast
all_years = sorted(df_forecast_adjusted['Year'].unique())

# Initialize the output dataframe with years
df_campus_output = pd.DataFrame({'YEAR': all_years})

print(f"\n=== Applying Disaggregation to Forecast ===")
print(f"Processing {len(all_years)} years from {min(all_years)} to {max(all_years)}")
print(f"Campus percentages available: {len(campus_percentages)}")

if len(campus_percentages) == 0:
    print("\n⚠️  WARNING: No campus percentages were calculated!")
    print("This means the disaggregation will fail. Check the previous section for errors.")

# Process each institution
for univ_code in df_forecast_adjusted['Univ_Code'].unique():
    print(f"\nProcessing: {univ_code}")

    # Get enrollment time series for this institution
    inst_forecast = df_forecast_adjusted[df_forecast_adjusted['Univ_Code'] == univ_code][['Year', 'Enrollment']].copy()
    inst_forecast = inst_forecast.set_index('Year')

    # Handle single-campus institutions
    if univ_code in SINGLE_CAMPUS.values():
        df_campus_output[univ_code] = df_campus_output['YEAR'].map(inst_forecast['Enrollment'])
        print(f"  ✓ Single campus - assigned to {univ_code}")

    # Handle multi-campus institutions
        ...
        print(f"  Multi-campus institution: {institution}")
        print(f"  Target campuses: {list(campus_map.values())}")
        ...
        for site, campus_code in campus_map.items():
            ...
            if percentage > 0:
                # Apply percentage to create campus-specific forecast
                df_campus_output[campus_code] = df_campus_output['YEAR'].map(
                    lambda year: int(round(inst_forecast.loc[year, 'Enrollment'] * percentage))
                    if year in inst_forecast.index else 0
                )
                print(f"    ✓ {campus_code}: {percentage:.1%} of total")
                campuses_assigned += 1
            else:
                df_campus_output[campus_code] = 0
                print(f"    ✗ {campus_code}: No percentage data (set to 0)")

        if campuses_assigned == 0:
            print(f"  ⚠️  WARNING: No campuses were assigned data for {univ_code}!")
        else:
            print(f"  Summary: {campuses_assigned}/{len(campus_map)} campuses assigned")

print(f"\n{'='*60}")
print(f"Generated {len(df_campus_output.columns)-1} campus columns")
print(f"Columns: {[c for c in df_campus_output.columns if c != 'YEAR']}")
```

## Add UVU Special Campuses (Placeholder)

```{python}
# UVU satellite campuses (Lehi, Payson, Vineyard) are handled separately
# For now, initialize as zero - these will be populated from a separate data source

# Initialize columns if they don't exist
for campus in ['UVU_Lehi', 'UVU_Payson', 'UVU_Vineyard']:
    if campus not in df_campus_output.columns:
        df_campus_output[campus] = 0

print("\nNote: UVU_Lehi, UVU_Payson, UVU_Vineyard initialized to 0")
print("These campuses require separate data source for enrollment projections")
```
...
COLUMN_ORDER = [
    ...
    'UOFU_Main',
    'UVU_Main', 'UVU_Lehi', 'UVU_Payson', 'UVU_Vineyard',
    'WSU_Main', 'WSU_Davis',
    'WESTMIN'
]
...
# Convert all enrollment columns to integers
for col in COLUMN_ORDER:
    if col != 'YEAR':
        df_campus_final[col] = df_campus_final[col].fillna(0).astype(int)

# Display preview
print("\n=== Campus-Level Enrollment Forecast (First 5 Years) ===")
print(df_campus_final.head())

print(f"\n=== Campus-Level Enrollment Forecast (Last 5 Years) ===")
print(df_campus_final.tail())

# Display summary statistics
print("\n=== Summary Statistics (2025) ===")
if 2025 in df_campus_final['YEAR'].values:
    row_2025 = df_campus_final[df_campus_final['YEAR'] == 2025].iloc[0]
    for col in COLUMN_ORDER:
        if col != 'YEAR':
            val = row_2025[col]
            print(f"{col}: {val:,}")
...
```{python}
# Export disaggregated campus-level forecast
campus_output_path = PATHS['forecast_output'].parent / 'enrollment_forecast_by_campus_2024_2060.csv'
df_campus_final.to_csv(campus_output_path, index=False)
print(f"\nCampus-level forecast saved to: {campus_output_path}")
```
...
* [enrollment_forecast_by_campus_2024_2060.csv](./intermediate/enrollment_forecast_by_campus_2024_2060.csv)
:::
