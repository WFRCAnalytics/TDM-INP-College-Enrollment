...
description: |
  ...
  **Weighted Participation Rate** method. The approach estimates each university's
  ...
  (Linear or Logarithmic) to that share, applies a regional control total, and
...
We evaluate two standard structural models for predicting the participation rates:
...
```{python}
# ──────────────────────────────────────────────────────────────────────────────
# Fit both Linear and Log Regression Models
# ──────────────────────────────────────────────────────────────────────────────
models_dict = {"Linear": {}, "Log": {}}
model_metrics = []
model_fit_data = []
...
for key in SCHOOL_KEYS:
    ...
    for mod_type in ["Linear", "Log"]:
        X_all = X_lin if mod_type == "Linear" else X_log
...
for mod_type in ["Linear", "Log"]:
    for key in SCHOOL_KEYS:
        ...
        t_2023 = 2023 - 1999
        X_2023 = np.array([[t_2023]]) if mod_type == "Linear" else np.array([[np.log(t_2023)]])
...
## Step 6 & 7 — Final Forecasts (Linear and Log)
...
for mod_type in ["Linear", "Log"]:
    for year in FORECAST_YEARS:
        t = year - 1999
        X_val = np.array([[t]]) if mod_type == "Linear" else np.array([[np.log(t)]])

        raw = {}
        for key in SCHOOL_KEYS:
            model = models_dict[mod_type][key]
            model_rate = float(model.predict(X_val)[0])
...
viewof selected_model = Inputs.radio(["Log", "Linear"], {
