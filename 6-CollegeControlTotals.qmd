...
# Filter to Fall 2025 term only for consistency
df_ushe_fall_2025 = df_ushe_2025[
    (df_ushe_2025['Academic Year'] == 2025) &
    (df_ushe_2025['Term'] == 'Fall')
].copy()

# Sum headcount by Institution and Site
...
print(f"Fall 2025 records: {len(df_ushe_fall_2025)}")
...
# Create disaggregated forecast records
disaggregated_records = []

for _, row in df_forecast_adjusted.iterrows():
    year = row['Year']
    ...
    # Handle multi-campus institutions
        ...
        for site, campus_code in campus_map.items():
            ...
            campus_enrollment = int(round(total_enrollment * percentage))
            record[campus_code] = campus_enrollment

        disaggregated_records.append(record)

# Combine all records by year
df_disaggregated = pd.DataFrame(disaggregated_records)
df_disaggregated = df_disaggregated.groupby('YEAR').sum().reset_index()

print(f"\nDisaggregated {len(df_disaggregated)} years of forecasts")
print(f"Campus columns: {[c for c in df_disaggregated.columns if c != 'YEAR']}")
```
...
for campus in ['UVU_Lehi', 'UVU_Payson', 'UVU_Vineyard']:
    if campus not in df_disaggregated.columns:
        df_disaggregated[campus] = 0
...
for col in COLUMN_ORDER:
    if col not in df_disaggregated.columns:
        df_disaggregated[col] = 0

# Reorder and select only desired columns
df_campus_final = df_disaggregated[COLUMN_ORDER].copy()
