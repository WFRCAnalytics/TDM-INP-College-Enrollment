...
models_dict = {"Linear": {}, "Log": {}, "Logistic": {}}
model_metrics = []
model_fit_data = []
...
for key in SCHOOL_KEYS:
    ...
    for mod_type in ["Linear", "Log", "Logistic"]:
        X_all = X_lin if mod_type in ["Linear", "Logistic"] else X_log
        X_train = X_all[train_mask]
        X_test  = X_all[test_mask]

        if mod_type in ["Linear", "Log"]:
            # Fit Model
            ...
            try:
                ...
                popt, _ = curve_fit(
                    logistic_func, X_train.flatten(), rates_train,
        ...
        model_metrics.append({
            "Model": mod_type, "University": full_name, "Dataset": "Train",
        ...
        for yr, actual, pred in zip(years_train, enroll_act_train, enroll_pred_train):
            model_fit_data += [
                {"Model": mod_type, "Year": int(yr), "University": full_name, "Value": float(actual), "Type": "Actual (Train)"},
                {"Model": mod_type, "Year": int(yr), "University": full_name, "Value": float(pred),   "Type": "Predicted (Train)"},
            ]
        ...
        if test_mask.any():
            ...
            model_metrics.append({
                "Model": mod_type, "University": full_name, "Dataset": "Test",
                "Alpha": alpha, "Beta": beta, "R2": r2_test, "MAPE": mape_test
            })

            for yr, actual, pred in zip(years_test, enroll_act_test, enroll_pred_test):
                model_fit_data += [
                    {"Model": mod_type, "Year": int(yr), "University": full_name, "Value": float(actual), "Type": "Actual (Test)"},
                    {"Model": mod_type, "Year": int(yr), "University": full_name, "Value": float(pred),   "Type": "Predicted (Test)"},
                ]
...
# ──────────────────────────────────────────────────────────────────────────────
shift_factors = {"Linear": {}, "Log": {}, "Logistic": {}}
shift_summary = []
...
for mod_type in ["Linear", "Log", "Logistic"]:
    for key in SCHOOL_KEYS:
        ...
        elif mod_type in ["Linear", "Logistic"]:
        ...
        shift_summary.append({
            "Model": mod_type, "School": DISPLAY_NAMES[key],
...
for mod_type in ["Linear", "Log", "Logistic"]:
    for year in FORECAST_YEARS:
        ...
        for key in SCHOOL_KEYS:
            ...
            shifted_rate = model_rate + shift_factors[mod_type][key]
...
viewof selected_model = Inputs.radio(["Log", "Linear", "Logistic"], {
...
Plot.plot({
  marks: [
    ...
    Plot.line(data_final_forecast, {
      ...
      strokeDasharray: d => d.Type === "Projected" ? "5,5" : "none"
    }),
